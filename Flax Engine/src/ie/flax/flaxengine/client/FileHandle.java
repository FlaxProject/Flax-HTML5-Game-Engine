...
/**
 * This class handles files (surprise!). You can read and write files from a server specified in {@link Settings}, or
 * html5 local storage. You can also clear local storage and delete key-value pairs.
 * 
 * @author carllange
 */
public class FileHandle {
		  ...
		  Request request = builder.sendRequest(null, new RequestCallback() {
		    ...
		    public void onResponseReceived(Request request, Response response) {
		      if (200 == response.getStatusCode()) {
		    	 ...
		    	 FLog.error("An error occurred accessing the server. It was a " + response.getStatusCode());
		      }
		    }       
		  });
	...
	/**
	 * This writes a string to local storage. It will log, but otherwise quietly fail if local storage does not exist.
	 * 
	 * @param key The key to the key-value-pair in local storage.
	 * @param stringToWrite The value to the key-value-pair in local storage.
	 */
	public static void writeStringToLocalStorage(String key, String stringToWrite){
		if (storage != null){
	 ...
	 * It will log if local storage does not exist. It will log if the key given does not exist or if its value is null.
	 * 
	 * @param key The key to the key-value-pair in local storage.
	 */
	...
	/**
	 * This removes a string from local storage. It will log, but otherwise quietly fail if local storage does not exist.
	 * 
	 * @param key The key to the key-value-pair in local storage.
	 */
